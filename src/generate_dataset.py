...
num_timesteps_per_simulation = 106
image_size = 64
...
parameter_space = {
    "num_balls":range(1,4),
    "gx":dists.uniform(0,0.4),  # As long as at least one parameter is a distribution, the sampler will sample WITHOUT REPLACEMENT (important)
...
try:
    ...
    pass
except: # Check twice but allow folder deletion
    rm = 'm'
    rm = input(f'Folder {args.output_path} exists.\nWould you like to replace it? (y/n)\n')
    if rm not in ['y','n']:
        print(f'{rm} invalid option, exiting')
        sys.exit()
    if rm == 'y':
        rm = input('Are you sure? (y/n)\n')
        if rm not in ['y','n']:
            print(f'{rm} invalid option, exiting')
            sys.exit()
        if rm == 'y':
...
sampler = ParameterSampler(parameter_space, args.number_of_simulations)
